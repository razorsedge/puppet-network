...
describe 'network::if::alias::range', :type => 'define' do
  ...
  context 'incorrect value: ensure' do
    ...
    it 'should fail' do
      expect {should contain_file('ifcfg-eth77-range0')}.to raise_error(Puppet::Error, /\$ensure must be either "up", "down", or "absent"./)
    end
  end
  ...
  context 'incorrect value: ipaddress_start' do
    ...
    it 'should fail' do
      expect {should contain_file('ifcfg-eth77-range0')}.to raise_error(Puppet::Error, /notAnIP is not an IP address./)
    end
  end
  ...
  context 'required parameters: ensure => down' do
    ...
    let(:facts) {{ :osfamily => 'RedHat' }}
    it { should contain_file('ifcfg-eth7-range9').with(
      :ensure => 'present',
      :mode   => '0644',
      :owner  => 'root',
      :group  => 'root',
      :path   => '/etc/sysconfig/network-scripts/ifcfg-eth7-range9',
      :notify => 'Service[network]'
    )}
    it 'should contain File[ifcfg-eth7-range9] with required contents' do
      verify_contents(subject, 'ifcfg-eth7-range9', [
  ...
  context 'required parameters: ensure => absent' do
    ...
    let(:facts) {{ :osfamily => 'RedHat' }}
    it { should contain_file('ifcfg-eth6-range9').with(
      :ensure => 'absent',
      :path   => '/etc/sysconfig/network-scripts/ifcfg-eth6-range9',
      :notify => 'Service[network]'
    )}
    it { should contain_service('network') }
  end
